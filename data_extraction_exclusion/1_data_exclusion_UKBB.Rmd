...
We combine some of the self reported PA data for instances 1 (2012-2013) and instances 2 (2014+)
```{r}
...
### PA4 prep:
Combining the newly created duration_moderate_activity and duration_vigorous_activity
we create the "MVPA_min_PA4" by adding both columns

```{r}

full_df <- full_df %>%
  mutate(
    MVPA_min_PA4 = coalesce(duration_vigorous_activity) + coalesce(duration_moderate_activity)
  )
