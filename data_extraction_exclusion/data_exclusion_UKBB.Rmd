...
full_df <- read.csv("/home/hiroshi/projects/UKBB_anaysis/AcclerometerProcessing/final_df_full.csv") # copy in /home/yacine/survival_analysis

```

############ Data prep intro ############

We combine some of the PA data for instances 1 (2012-2013) and instances 2 (2014+)
```{r}
full_df <- full_df %>%
  mutate(
...
full_df <- full_df %>%
  dplyr::select(-diabetes_1, -diabetes_2, -diabetes_3, 
         -processed_meat_1, -processed_meat_2, -processed_meat_3, -X191.0.0.1, -duration_moderate_activity_0, -duration_moderate_activity_3, -duration_vigorous_activity_0, -duration_vigorous_activity_3, -duration_heavy_DIY_0, -duration_heavy_DIY_3, -duration_light_DIY_0, -duration_light_DIY_3)
```
...
full_df <- full_df %>%
  mutate(education_level = case_when(
    ...
    education_level %in% c("A levels/AS levels or equivalent", 
                         ...
                         "A levels/AS, NVQ/HND/HNC or equivalent",
...
full_df$avg_hh_income_raw <- full_df$avg_hh_income
...
```{r}
full_df$avg_hh_income[full_df$avg_hh_income == "NA"] <- NA # some NA values remained
# in the avg_hh_income column so I make sur all NA values are marked as "NA"

# Yacine Note: There is an issue with hh income NA values, if I run only this chunk
# they are all removed but if we run the whole thing they stay... will fix
...
full_df <- full_df[complete.cases(full_df[, keep_cols]) & !is.na(full_df$diabetes) & !is.na(full_df$avg_hh_income), ]
result <- count_exclusions(df_before, full_df)
...
df_before <- full_df
keep_cols <- c('smoking', 'alcohol', 'ethnicity', 'tdi', 'education_level', 'oily_fish', 'fresh_fruit', 'cooked_vg', 'BMI', 'red_meat', 'non_oily_fish', 'processed_meat')
