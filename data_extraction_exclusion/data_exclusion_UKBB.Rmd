...
Check the min and max date of diagnosis among retained sample, last date of diagnosis among removed sample
```{r}
min(full_df$stroke_date, na.rm = T)
max(full_df$stroke_date, na.rm = T)
min(full_df$myocardial_infarction_date, na.rm = T)
max(full_df$myocardial_infarction_date, na.rm = T)

# Confirm the last date of diagnosis among those who are removed
df_beforeExclusion %>% 
  anti_join(full_df, by =join_by(eid)) %>% 
...
full_df <- full_df %>%
  mutate(fresh_fruit = case_when(
    ...
    fresh_fruit >= 2 & fresh_fruit < 3 ~ "2 or 3 servings/day",
    fresh_fruit >= 3 & fresh_fruit < 4 ~ "3 or 4 servings/day",
...
full_df <- full_df %>%
  mutate(cooked_vg = case_when(
    ...
    cooked_vg >= 2 & cooked_vg < 3 ~ "2 or 3 servings/day",
    cooked_vg >= 3 & cooked_vg < 4 ~ "3 or 4 servings/day",
...
full_df <- full_df %>%
  mutate(alcohol = case_when(
    alcohol_raw == "Daily or almost daily" ~ "Daily or almost daily",
    alcohol_raw == "Never" ~ "Never",
    alcohol_raw == "Once or twice a week" ~ "Once or twice a week",
    alcohol_raw %in% c("One to three times a month", "Special occasions only") ~ "Less than once a week",
    alcohol_raw == "Prefer not to answer" ~ "Prefer not to answer",
    TRUE ~ NA_character_
  ))
...
## we replace the codes by their meaning:
full_df <- full_df %>%
  mutate(diabetes_0 = recode(as.character(diabetes_0), !!!diabetes_coding))

full_df$diabetes <- full_df$diabetes_0
full_df <- full_df %>%
  select(-diabetes_0)
...
#hiroshiComment - where is the category - three or four times a week? for alcohol?
...
```{r}
rm_cols <- c("alcohol", "fresh_fruit", "education_level", "diabetes")
