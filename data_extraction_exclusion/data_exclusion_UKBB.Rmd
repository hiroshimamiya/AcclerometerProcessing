...
dim(full_df)
full_df$quality_good_calibration %>% table
...
# cutoff date indicating start of follow-up, CVD before these dates need to bre removed 
...
NOTE: This is the original script, with fixed baselione (acceleormter) date, which should not be used in favor of person-specific date for accelerometer 
```{r}
...
table(full_df$education, full_df$education_level, useNA = "always" )




#Grouping of categories and  making category name much shorter, from the surivial anlaysis script 
#recode edut to reduce the length of text (width of table)
full_df <- full_df %>%  
  dplyr::mutate(education_level = ifelse(education_level == "A levels/AS levels or equivalent, NVQ or HND or HNC or equivalent, other professional qualifications", "A/AS levels, NVQ or HND", education_level))

full_df <- full_df %>%  
  dplyr::mutate(education_level = ifelse(education_level == "O levels/GCSEs or equivalent, CSEs or equivalent", "O/GCSE levels or CSEs", education_level))

# ORder educ variable such that the order of category in the table will be correct
full_df$education_level <- factor(full_df$education_level, 
  levels = c(
    "College or University degree",
    "O/GCSE levels or CSEs",
    "A/AS levels, NVQ or HND",
    "None of the above"
  )
)

table(full_df$education_level, full_df$education_raw)
...
# Add date of birth
full_df$approx_dob <-
  as.Date(paste(full_df$year_birth, full_df$month_birth, "15", sep = "-"), c("%Y-%m-%d")) 

# UK Biobank doesn't contain day of birth as it would be unnecessary identifying information, so we roughly impute it as the 15th of the birth month.
...
```{r}
df_before <- full_df
keep_cols <- c('smoking_raw', 'alcohol_raw', 'ethnicity_raw', 'tdi_raw', 'education_raw', 'oily_fish', 'fresh_fruit', 'cooked_vg', 'BMI_raw', 'sex', 'beef')
#Hiroshi comment, no processed meat
full_df <- full_df[complete.cases(full_df[, keep_cols]), ] # Removal of missigness (NA)
result <- count_exclusions(df_before, full_df)
