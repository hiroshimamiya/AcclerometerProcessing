...
full_df <- full_df %>%
  select(-diabetes_1, -diabetes_2, -diabetes_3, 
         -processed_meat_1, -processed_meat_2, -processed_meat_3, -X191.0.0.1)
...
full_df <- full_df %>%
  rowwise() %>%
  mutate(red_meat = sum(c_across(c(beef, lamb, pork)), na.rm = TRUE))
...
full_df <- full_df %>%
  mutate(fresh_fruit = case_when(
    fresh_fruit == "Less than one serving/day" ~ "Zero servings/day",
    fresh_fruit == "Do not know" ~ "Do not know",
    fresh_fruit == "Prefer not to answer" ~ "Prefer not to answer",
    fresh_fruit == 0 ~ "< 2 servings/day",
...
full_df <- full_df %>%
  mutate(cooked_vg = case_when(
    cooked_vg == "Less than one serving/day" ~ "Zero servings/day",
    cooked_vg == "Do not know" ~ "Do not know",
    cooked_vg == "Prefer not to answer" ~ "Prefer not to answer",
    cooked_vg == 0 ~ "< 2 servings/day",
...
##Yacine Note: I save the df up to this point because the previous steps take too
much time to generate, we can remove from final version:
```{r}
write.csv(full_df, "/home/yacine/UKBB_beluga/full_recode.csv", row.names = FALSE)
```


```{r}
full_df <- read.csv("/home/yacine/UKBB_beluga/full_recode.csv")
...
############ Final removals ############
We remove all rows where any column contains the entry "Prefer not to answer" and
"Don't know":
```{r}
## we transform all the values marked as "Prefer not to answer" and "Don't know" as NAs:
answers_to_remove <- c("Prefer not to answer", "Don't know")
...
df_before <- full_df
keep_cols <- c('smoking', 'alcohol', 'ethnicity', 'tdi', 'education', 'oily_fish', 'fresh_fruit', 'cooked_vg', 'BMI', 'red_meat', 'non_oily_fish', 'processed_meat')
full_df <- full_df[complete.cases(full_df[, keep_cols]), ]
