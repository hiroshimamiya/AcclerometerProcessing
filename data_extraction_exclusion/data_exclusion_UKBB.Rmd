...
```{r}
#write.csv(full_df, "/home/yacine/UKBB_beluga/df_exc1.csv", row.names = FALSE)
```
...
full_df <- full_df %>%
  mutate(fresh_fruit = case_when(
    fresh_fruit == -10 ~ "Less than one serving/day",
...
full_df <- full_df %>%
  mutate(cooked_vg = case_when(
    cooked_vg == -10 ~ "Less than one serving/day",
...
table(full_df$fresh_fruit, full_df$cooked_vg_raw, useNA = "always")
...
```{r}
df_before <- full_df
keep_cols <- c('smoking_raw', 'alcohol_raw', 'ethnicity_raw', 'tdi_raw', 'education_raw', 'oily_fish', 'fresh_fruit', 'cooked_vg', 'BMI_raw', 'sex', 'month_birth', 'year_birth', 'diabetes')
