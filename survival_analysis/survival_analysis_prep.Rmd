...
# rename MVPA quarters
full_df_incl_PA1_PA2 <- full_df_incl_PA1_PA2 %>%
  mutate(MVPA_Quant_PA2 = factor(MVPA_Quant_PA2,
                                 levels = c('[0,99]', '(99,222]', '(222,395]', '(395,1.01e+04]'),
                                 labels = c('MVPA Quarter 1', 'MVPA Quarter 2', 'MVPA Quarter 3', 'MVPA Quarter 4')))


# we create the summary baseline characteristics table
summ_base_char <- full_df_incl_PA1_PA2 %>%
  select(sex, fresh_fruit, oily_fish, cooked_vg, processed_meat, alcohol, 
         smoking, ethnicity, education_level, stroke, MI, diabetes, BMI, 
         MVPA_min_PA1, MVPA_min_PA2, MVPA_Quant_PA2) %>%
  tbl_summary(
    by = MVPA_Quant_PA2,
...
```{r}
full_df$MVPA_quarters <- qtile_cut(full_df$MVPA_min_PA2, labels = c("Quarter 1", "Quarter 2", "Quarter 3", "Quarter 4"))
```
