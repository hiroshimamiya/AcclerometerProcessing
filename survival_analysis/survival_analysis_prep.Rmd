...
dfPA1Label <- data.frame(rangeMin = full_df_PA$MVPA_Quant_PA1 %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
dfPA2Label <- data.frame(rangeMin = full_df_PA$MVPA_Quant_PA2 %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
...
data.frame(full_df_PA$MVPA_Quant_PA1 %>% unique(), Q = c("Q1", "Q2", "Q3", "Q4"))
dfPA2Label <- data.frame(range = full_df_PA$MVPA_Quant_PA2 %>% unique(), Q = c("Q1", "Q2", "Q3", "Q4"))
...
dfPA1Label <- data.frame(rangeMin = full_df_PA$MVPA_Quant_PA1_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
dfPA2Label <- data.frame(rangeMin = full_df_PA$MVPA_Quant_PA2_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
```
...
dfPA1Label <- data.frame(range = full_df_PA$MVPA_Quant_PA1 %>% unique(), Q = c("Q1", "Q2", "Q3", "Q4"))
dfPA2Label <- data.frame(range = full_df_PA$MVPA_Quant_PA2 %>% unique(), Q = c("Q1", "Q2", "Q3", "Q4"))
...
tb3 <- as_flex_table(tb3)
tb3 <- width(tb3, j = 1:4, width = c(1,1,1,1), unit = "cm")
...
# we create function to transform KM model to Log-log function:
# we create a function to transform the KM model to Log-log function
>>>>>>> e18f7e7 (added code for creating and ploting log log function of the KM model as well as plotting the Schoenfeld residuals)
...
<<<<<<< HEAD
# we create fuction to create Log-log survival plots:
plot_log_log_survival <- function(log_log_data, title) {
  ggplot(log_log_data, aes(x = time, y = log_log_surv, color = strata)) +
    geom_step() +
    labs(title = title, x = "Time", y = "Log-log Survival Function") +
    theme_minimal()
}

# Plot each log-log transformed survival curve
plot1_mi <- plot_log_log_survival(log_log_kmfit1_mi, "Log-Log Survival Curve for MI per ENMO by Quarters")
plot1_stroke <- plot_log_log_survival(log_log_kmfit1_stroke, "Log-Log Survival Curve for Stroke per ENMO by Quarters")
plot2_mi <- plot_log_log_survival(log_log_kmfit2_mi, "Log-Log Survival Curve for MI per ML by Quarters")
plot2_stroke <- plot_log_log_survival(log_log_kmfit2_stroke, "Log-Log Survival Curve for Stroke per ML by Quarters")
=======
# we create a function to create Log-log function plots
plot_log_log_survival <- function(log_log_data, title) {
  ggplot(log_log_data, aes(x = time, y = log_log_surv, color = strata)) +
    geom_step() +
    labs(title = title, x = "Time in Month", y = "Log-log Survival Function") +
...
# we test the ph assumption of a Cox regression model
...
# we plot the Schoenfeld residuals:
=======
