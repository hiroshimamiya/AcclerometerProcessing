...
full_df_incl_PA1_PA2$education_level <- factor(full_df_incl_PA1_PA2$education_level, 
  levels = c(
...
table(full_df_incl_PA1_PA2$education_level)



# Did we agree to compress all categories into two for processed meat and oily fish? Where are categories less than 2 servings?
...
tb <- dt %>%
  tbl_summary(
    by = MVPA_Quant_PA2,  
    all_continuous() ~ "{median} c({p25}, {p75})",
...
summ_base_char_MVPA_quarter1 <- df_MVPA_quarter1 %>%
  ...
  kable_styling(latex_options = c("scale_down", "hold_position"), 
                font_size = 8) # Adjust font size to fit better

# Print the table
knitr::knit_print(summ_base_char_MVPA_quarter1)
```
...
knitr::knit_print(summ_base_char_MVPA_quarter2)
```
...
knitr::knit_print(summ_base_char_MVPA_quarter3)
```
...
knitr::knit_print(summ_base_char_MVPA_quarter4)
```
...
fitDf <- coxph(Surv(fu_time, MI) ~ 
                 ...
                 processed_meat + 
                 cooked_vg + 
                 oily_fish, data = full_df_incl_PA1_PA2)
