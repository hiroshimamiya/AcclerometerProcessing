...
```{r}
funcPlotForest2 <- function(forest_data, title = "Forest Plot", subtitle = "Hazard Ratios and 95% Confidence Intervals", maxX = 2, new_titles = NULL) {
  
  # Ensure no NA values and modify 'index' column
  forest_data <- forest_data %>%
    filter(!is.na(est) & !is.na(lwr) & !is.na(upr))
  
  # Overwrite 'index' with custom labels if provided
  if (!is.null(new_titles)) {
    forest_data$index <- new_titles
  }
  
  # Convert 'index' to factor to ensure ordering
  forest_data <- forest_data %>%
    mutate(
      index = factor(index, levels = rev(unique(index))),
      foodName = factor(foodName, levels = c("Vector Magnitude (ENMO)", "Random Forest and HMM (ML)", "Activity Count", "Self-Report (IPAQ)"))
    )
  ...
  ggplot(data = forest_data, aes(y = index, x = est, xmin = lwr, xmax = upr, color = foodName)) + 
    ...
    scale_color_manual(values = c(
      "Vector Magnitude (ENMO)" = "blue",
      "Random Forest and HMM (ML)" = "red",
...
new_titles <- c(
  "Vector Magnitude (ENMO) - Q2",
  "Vector Magnitude (ENMO) - Q3",
  "Vector Magnitude (ENMO) - Q4",
  "Machine Learning (ML) - Q2",
  "Machine Learning (ML) - Q3",
  "Machine Learning (ML) - Q4",
  "Activity Count - Q2",
  "Activity Count - Q3",
  "Activity Count - Q4"
)
...
f_cov <- "Sex + Age + Race + `Deprivation index` + Education + `Household Income` + Smoking + Alcohol + `Processed meat` + `Fresh fruit` + `Cooked vegetables` + `Oily fish` + `Non-oily fish` + `Greenspace 300m from residence` + `Water 300m from residence` +`Natural env. 300m from residence`"

f_cov_noMediator <- "Sex + Age + Race + `Deprivation index` + Education + `Household Income` + Smoking + Alcohol + `Processed meat` + `Fresh fruit` + `Cooked vegetables` + `Oily fish` + `Non-oily fish` + `Greenspace 300m from residence` + `Water 300m from residence` +`Natural env. 300m from residence`"
...
data_t1 <- data_t1 %>%
  ...
  mutate(
    foodName = "Vector Magnitude (ENMO)"  
  )
...
png("rf_sensa5.png", width = 800, height = 600)
funcPlotForest2(forest_data,
                title = "Sensitivity Analysis 5 - Forest Plot (Main) Stroke", 
...
data_t1 <- data_t1 %>%
  ...
  mutate(
    foodName = "Vector Magnitude (ENMO)"  # or another identifier if applicable
  )
...
# we can now create the plot using the funcPlotForest function created earlier:
png("rf_sensa5_MI.png", width = 800, height = 600)
funcPlotForest2(forest_data,
                title = "Sensitivity Analysis 5 - Forest Plot (Main) MI", 
                subtitle = "HR for MVPA Quartiles with 95% CI", 
                maxX = 1.5, 
                new_titles = new_titles)
dev.off()
...
### Sensitivity 1 - no Mediator , withoubt BMI and Diabetes , Stroke 
...
fit3 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA3 + ",  f_cov_noMediator)), data = dt)
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3"
                    )  #%>%  
  #modify_table_body(
  #  ~.x %>% 
  #    mutate(
...
# Landscape (word format)
tbl_merge_sensa1_stroke <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Without BMI and Diabetes" = tbl_merge_sensa1_stroke,  
...
data_t3 <- as.data.frame(t3$table_body)
...
#======== This step is to change the variable names but I wasn't able to do 
#======== it without messing up the order of the plot lines 
...
### Sensitivity 1 - no Mediator , withoubt BMI and Diabetes , MI
```{r}
# Model for PA 1 
...
fit3 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA3 + ",  f_cov_noMediator)), data = dt)
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3"
                    )  
...
# Landscape (word format)
tbl_merge_sensa1_MI <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Without BMI and Diabetes" = tbl_merge_sensa1_MI,  
  #path = "HR_Sens_Diabetes_Stroke.docx", 
  #pr_section = sect_properties
#)  
```
...
### Sensitivity 2 - Removed people with huge duration (>1200min) of MVPA Stroke 
```{r}

# Model for PA 1 
fit1 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA1_truncated + ",  f_cov)), data = dt)
t1<- tbl_regression(fit1, exponentiate = TRUE, 
                    label = list("MVPA_Quant_PA1_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA1_truncated"
                    ) 

# Model for PA 2
fit2 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA2_truncated + ",  f_cov)), data = dt)
t2<- tbl_regression(fit2, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA2_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA2_truncated"
                    )  

# Model for PA 3
fit3 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA3_truncated + ",  f_cov)), data = dt)
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3_truncated"
                    )  
...
# Landscape (word format)
tbl_merge_sensa2_stroke <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Hazard ratios for icidence stroke, truncated MVPA" = tbl_merge_sensa2_stroke,  
...
### Sensitivity 2 - Removed people with huge duration (>1200min) of MVPA, MI 
```{r}

# Model for PA 1 
fit1 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA1_truncated + ",  f_cov)), data = dt)
t1<- tbl_regression(fit1, exponentiate = TRUE, 
                    label = list("MVPA_Quant_PA1_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA1_truncated"
                    ) 

fit2 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA2_truncated + ",  f_cov)), data = dt)
t2<- tbl_regression(fit2, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA2_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA2_truncated"
                    )  

fit3 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA3_truncated + ",  f_cov)), data = dt)
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3_truncated"
                    )  
...
# Landscape (word format)
tbl_merge_sensa2_MI <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Hazard ratios for incidence MI, truncated MVPA" = tbl_merge_sensa2_MI,  
...
# Model for PA 1 
fit1 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA1_truncated + ",  gsub(x = f_cov, pattern = "Sex \\+", replacement = ""))), data = dt[dt$Sex == "Female", ])
t1<- tbl_regression(fit1, exponentiate = TRUE, 
                    label = list("MVPA_Quant_PA1_truncated" = "MVPA Quartile_truncated"), 
                    include = "MVPA_Quant_PA1_truncated"
                    ) 
...
fit3 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA3 + ",  gsub(x = f_cov, pattern = "Sex \\+", replacement = ""))), data = dt[dt$Sex == "Female", ])
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3"
                    )  
...
# Landscape (word format)
tbl_merge_sensa3_stroke_f <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Female - Hazard ratios for incidence stroke" = tbl_merge_sensa3_stroke_f ,  
...
# Model for PA 3
fit3 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA3 + ",  gsub(x = f_cov, pattern = "Sex \\+", replacement = ""))), data = dt[dt$Sex == "Male", ])
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3"
                    )  
...
# Landscape (word format)
tbl_merge_sensa3_stroke_m <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Male - Hazard ratios for incidence Stroke" = ttbl_merge_sensa3_stroke_m,  
...
# Model for PA 3
fit3 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA3 + ",  gsub(x = f_cov, pattern = "Sex \\+", replacement = ""))), data = dt[dt$Sex == "Female", ])
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3"
                    )  
...
# Landscape (word format)
tbl_merge_sensa3_MI_f <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Female - Hazard ratios for incidence MI" = tbl_merge_sensa3_MI_f,  
...
# Model for PA 3
fit3 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA3 + ",  gsub(x = f_cov, pattern = "Sex \\+", replacement = ""))), data = dt[dt$Sex == "Male", ])
t3<- tbl_regression(fit3, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA3" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA3"
                    )  
...
# Landscape (word format)
tbl_merge_sensa3_MI_m <- as_flex_table(tbl_merge)
sect_properties <- prop_section(
  page_size = page_size(
  orient = "portrait",
  #width = 8, height = 11.7
...
# Save 
#flextable::save_as_docx("Male - Hazard ratios for incidence MI" = tbl_merge_sensa3_MI_m,  
...
Interaction of MVPA and Male/Female # Note Yacine: Did you add this?
Female 
```{r}
...
t3 <- tbl_merge(
  tbls = list(t1, t2),
  tab_spanner = c("**Vector Magnitude (ENMO)**", "**Random Forest and HMM**")
)
...
# Model for PA 1 
fit1 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA1 + ",  f_cov)), data = dt_sa4)
t1<- tbl_regression(fit1, exponentiate = TRUE, 
                    label = list("MVPA_Quant_PA1" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA1"
                    ) 
...
# Model for PA 2
fit2 <- coxph(as.formula(paste(f_stroke, " ~" ,  "MVPA_Quant_PA2 + ",  f_cov)), data = dt_sa4)
t2<- tbl_regression(fit2, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA2" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA2"
                    )  
...
# Model for PA 1 
fit1 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA1_truncated + ",  f_cov)), data = dt_sa4)
t1<- tbl_regression(fit1, exponentiate = TRUE, 
                    label = list("MVPA_Quant_PA1_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA1_truncated"
                    ) 
...
# Model for PA 2
fit2 <- coxph(as.formula(paste(f_MI, " ~" ,  "MVPA_Quant_PA2_truncated + ",  f_cov)), data = dt_sa4)
t2<- tbl_regression(fit2, exponentiate = TRUE,  
                    label = list("MVPA_Quant_PA2_truncated" = "MVPA Quartile"), 
                    include = "MVPA_Quant_PA2_truncated"
                    )  
...
surv_analysis_doc <- surv_analysis_doc %>%
  ...
  body_add_flextable(tbl_merge_sensa1_stroke) %>%
...
surv_analysis_doc <- surv_analysis_doc %>%
  ...
  body_add_flextable(tbl_merge_sensa1_MI) %>%
...
surv_analysis_doc <- surv_analysis_doc %>%
  ...
  body_add_flextable(tbl_merge_sensa2_stroke) %>%
...
surv_analysis_doc <- surv_analysis_doc %>%
  ...
  body_add_flextable(tbl_merge_sensa2_MI) %>%
...
surv_analysis_doc <- surv_analysis_doc %>%
  ...
  body_add_flextable(tbl_merge_sensa3_stroke_f) %>%
...
surv_analysis_doc <- surv_analysis_doc %>%
  ...
  body_add_flextable(tbl_merge_sensa3_stroke_m) %>%
...
legend("bottomleft", c("[0.167,319]", "(319,464]", "(464,641]", "(641,2.39e+03]"), cex=0.5,
       col = c("black", "grey", "blue", "red"), lty = 1)
title(main = "MI - Vector Magnitude (ENMO) by Quarters")
...
legend("bottomleft", c("[0.167,319]", "(319,464]", "(464,641]", "(641,2.39e+03]"), cex=0.5, 
       col = c("black", "grey", "blue", "red"), lty = 1)
title(main = "Stroke - Vector Magnitude (ENMO) by Quarters")
...
plot1_mi <- plot_log_log_survival(log_log_kmfit1_mi, "Log-Log Survival Curve for MI per Vector Magnitude (ENMO) Quarters Comparison")
plot1_stroke <- plot_log_log_survival(log_log_kmfit1_stroke, "Log-Log Survival Curve for Stroke per Vector Magnitude (ENMO) Quarters Comparison")
...
ggcoxzph(test_ph_fit1, var = 'MVPA_Quant_PA1_truncated')
ggcoxzph(test_ph_fit2, var = 'MVPA_Quant_PA2_truncated')
