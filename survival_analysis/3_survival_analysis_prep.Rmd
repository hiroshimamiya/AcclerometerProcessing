...
dt$MVPA_Quant_PA1 <- as.numeric(dt$MVPA_Quant_PA1)
dt$MVPA_Quant_PA2 <- as.numeric(dt$MVPA_Quant_PA2)
dt$MVPA_Quant_PA3 <- as.numeric(dt$MVPA_Quant_PA3)
...
fit1_smooth_stroke <- coxph(Surv(`Follow_up time`, Stroke) ~ 
                       bs(MVPA_Quant_PA1, df = 4), data = dt)

fit1_smooth_MI <- coxph(Surv(`Follow_up time`, `Myocardial Infarction`) ~ 
                       bs(MVPA_Quant_PA1, df = 4), data = dt)

par(ask = FALSE) 
termplot(fit1_smooth_stroke, se = TRUE, partial.resid = TRUE, main = "Stroke PA1")
