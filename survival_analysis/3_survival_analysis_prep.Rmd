...
funcPlotForest <- function(forest_data, title = "Forest Plot", subtitle = "Hazard Ratios and 95% Confidence Intervals", maxX = 2) {
  ...
  ggplot(data = forest_data, aes(y = index, x = est, xmin = lwr, xmax = upr, color = foodName)) + 
    ...
    scale_color_manual(values = c("ENMO" = "blue", "Random Forest and HMM (ML)" = "red", "Activity Count" = "brown")) +  # Set colors for groups
    theme_classic() + 
    theme(
...
tbl_merge <- tbl_merge(
  tbls = list(t1, t2, t3),
  tab_spanner = c("**ENMO**", "**Random Forest and HMM**", "**Activity Count**")
) %>%  bold_labels() %>% 
