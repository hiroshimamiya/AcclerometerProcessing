...
funcPlotForest <- function(forest_data, title = "Forest Plot", subtitle = "Hazard Ratios and 95% Confidence Intervals", maxX = 2) {
  forest_data <- forest_data %>%
    ...
    mutate(index = factor(index, levels = rev(unique(index))))  # fix order (lowest to highest)
  ...
  ggplot(data = forest_data, aes(y = index, x = est, xmin = lwr, xmax = upr, color = foodName)) + 
    ...
    scale_color_manual(values = c("ENMO" = "blue", "Random Forest and HMM (ML)" = "red")) +  
    theme_classic() + 
    theme(
    ...
    theme(
      strip.text.y = element_text(size = 12, vjust = 1),
...
plot <- funcPlotForest(forest_data, 
