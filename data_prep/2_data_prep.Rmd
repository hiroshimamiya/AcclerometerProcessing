...
sum(full_df_PA$MVPA_min_PA4 > 1000)
summary(full_df_PA$MVPA_min_PA4)


dim(full_df_PA)
full_df_PA <- full_df_PA %>%  filter(MVPA_min_PA2 < 2500)
full_df_PA <- full_df_PA %>%  filter(MVPA_min_PA1 < 2500)
full_df_PA <- full_df_PA %>%  filter(MVPA_min_PA4 < 1000)
dim(full_df_PA)
...
full_df_PA$MVPA_Quant_PA4 <- dvmisc::quant_groups(full_df_PA$MVPA_min_PA4, 4) %>% factor()
...
dfPA4Label <- data.frame(range = full_df_PA$MVPA_Quant_PA4 %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
...
```{r}
# check plot to see if there are extreme data points or not 
boxplot(MVPA_min_PA4 ~ MVPA_Quant_PA4, data = full_df_PA, main = "Categorical MVPA CV Numeric MVPA")
boxplot(MVPA_min_PA2 ~ MVPA_Quant_PA2, data = full_df_PA, main = "Categorical MVPA CV Numeric MVPA")
...
# get the slope 
reg <- lm(MVPA_min_PA2 ~ MVPA_min_PA4, data = full_df_PA)
...
full_df_PA$MVPA_Quant_PA4_trucated <- dvmisc::quant_groups(full_df_PA$MVPA_min_PA4_truncated, 4) %>% factor()
...
dfPA4LabelTrunc <- data.frame(range = full_df_PA$MVPA_Quant_PA4_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
```
...
# Convert varaible names into descruptive table names 
