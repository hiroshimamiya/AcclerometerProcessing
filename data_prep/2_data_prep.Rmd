...
### PA3 - we merge PA3 df to full_df
```{r}
PA3_proportions_data <- read_csv("/home/yacine/UKBB_beluga/PA3_df/combined_df_drop_duplicates.csv",col_select = -1)
...
sum(full_df_PA_2$MVPA_min_PA4 > 1000)
...
full_df_PA_2 <- full_df_PA_2 %>%  filter(MVPA_min_PA4 < 1000)
dim(full_df_PA_2)
...
ggplot(full_df_PA_2, 
  ...
  geom_point(color="black", size = 0.3) +
  theme_classic() + 
  xlab("MVPA minutes / week - Self-Reported") + 
  ylab("MVPA minutes / week - ML") +  
  geom_smooth(method=lm, 
    se=FALSE, 
    linetype="dashed",
    color="grey",
    main = "Self-Reported and ML"
  ) +
  ggtitle("Self-Reported and ML")
...
full_df_PA$MVPA_Quant_PA3_trucated <- dvmisc::quant_groups(full_df_PA$MVPA_min_PA3_truncated, 4) %>% factor()
...
dfPA3LabelTrunc <- data.frame(range = full_df_PA$MVPA_Quant_PA3_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
```
...
full_df_PA_2$MVPA_Quant_PA1_trucated <- dvmisc::quant_groups(full_df_PA_2$MVPA_min_PA1_truncated, 4) %>% factor()
full_df_PA_2$MVPA_Quant_PA2_trucated <- dvmisc::quant_groups(full_df_PA_2$MVPA_min_PA2_truncated, 4) %>% factor()
full_df_PA_2$MVPA_Quant_PA3_trucated <- dvmisc::quant_groups(full_df_PA_2$MVPA_min_PA3_truncated, 4) %>% factor()
full_df_PA_2$MVPA_Quant_PA4_trucated <- dvmisc::quant_groups(full_df_PA_2$MVPA_min_PA4_truncated, 4) %>% factor()


dfPA1LabelTrunc <- data.frame(range = full_df_PA_2$MVPA_Quant_PA1_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
dfPA2LabelTrunc <- data.frame(range = full_df_PA_2$MVPA_Quant_PA2_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
dfPA2LabelTrunc <- data.frame(range = full_df_PA_2$MVPA_Quant_PA3_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
dfPA4LabelTrunc <- data.frame(range = full_df_PA_2$MVPA_Quant_PA4_trucated %>% levels(), Q = c("Q1", "Q2", "Q3", "Q4"))
```
...
dt <- full_df_PA %>%  dplyr::select(
  ...
  MVPA_Quant_PA3_trucated,
  #MVPA_Quant_PA4_trucated,  
...
# Save 
flextable::save_as_docx("Distribution of participant characteristics by Quartile of Moderate_Vigorous Physical Activity" = tb,  
               path = "testTable1.docx", 
               pr_section = sect_properties
               )  
```
## Table 1.2 (incudes PA4)
```{r}
...
tb2 <- dt2 %>% 
  dplyr::select(-contains("trucated")) %>% 
  dplyr::select( -MVPA_Quant_PA1, -MVPA_Quant_PA3, -MVPA_Quant_PA4) %>% 
  tbl_summary(
    by = MVPA_Quant_PA2,  
    #statistic = all_continuous() ~ "{mean} ({sd})"
    #digits = all_continuous() ~ 1,
...
# Save 
flextable::save_as_docx("Distribution of participant characteristics by Quartile of Moderate_Vigorous Physical Activity" = tb2,  
               path = "testTable.docx", 
               pr_section = sect_properties
)
...
saveRDS(dt, "/home/yacine/AcclerometerProcessing/prepped_dt.rds")
saveRDS(dt2, "/home/yacine/AcclerometerProcessing/prepped_dt2.rds")
...
par(mfrow = c(2,2))
hist(dt$`MVPA min/week`, xlab = "minutes", main = "Histogram \n Duration MVPA min")
hist(dt$`MVPA min/week ML`, xlab = "minutes", main = "Histogram \n Duration MVPA min - ML")
hist(dt$`MVPA min/week Activity Count`, xlab = "minutes", main = "Histogram \n Duration MVPA min - Activity Count")
hist(dt2$`MVPA min/week Self-Report`, xlab = "minutes", main = "Histogram \n Duration MVPA min - Self-Report")
dev.off()
