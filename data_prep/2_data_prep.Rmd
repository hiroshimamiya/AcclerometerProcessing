...
enmoTSParallel_PA1_v2 <- read_csv("/home/aayush/PA1_3_new_thresholds/beluga/PA1_combined_df.csv", col_select = -1)
...
PA3_proportions_data_v2 <- read_csv("/home/aayush/PA1_3_new_thresholds/beluga/PA3_L2_one_minute_without_duplicates.csv",col_select = -1)
...
full_df_PA_v2<- inner_join(full_df_PA_v2, 
  ML_derived_PA2 %>% 
    dplyr::select(eid, MVPA_min_PA2, LPA_min_PA2, SB_min_PA2),
  by = "eid")
...
#full_df_PA <- full_df_PA %>%  filter(MVPA_100 < 2500)
#full_df_PA <- full_df_PA %>%  filter(MVPA_150 < 2500)
#full_df_PA <- full_df_PA %>%  filter(MVPA_min_PA3_v2 < 2500)
dim(full_df_PA_2)
...
full_df_PA$overall_health_rating %>% table 
full_df_PA$overall_health_rating <- factor(full_df_PA$overall_health_rating)
levels(full_df_PA$overall_health_rating)
levels(full_df_PA$overall_health_rating) <- list( "Do not know/Prefer not to answer" = "Do not know/Prefer not to answer", 
                                    "Poor" =  "Poor",
                                    "Fair" = "Fair",
                                    "Good" = "Good",
                                    "Excellent" = "Excellent") 
...
full_df_PA_2$overall_health_rating %>% table 
full_df_PA_2$overall_health_rating <- factor(full_df_PA_2$overall_health_rating)
levels(full_df_PA_2$overall_health_rating)
levels(full_df_PA_2$overall_health_rating) <- list( "Do not know/Prefer not to answer" = "Do not know/Prefer not to answer", 
                                    "Poor" =  "Poor",
                                    "Fair" = "Fair",
                                    "Good" = "Good",
                                    "Excellent" = "Excellent") 
...
### Covariates - reorder for full_df_PA_v2 df

```{r}
# Re-order categories 
full_df_PA_v2$cooked_vg %>% table
full_df_PA_v2$cooked_vg <- factor(full_df_PA_v2$cooked_vg)
levels(full_df_PA_v2$cooked_vg)
levels(full_df_PA_v2$cooked_vg) <- list( 'Less than 2 servings a day' ="< 2 servings/day", 'Between 2 and 4 servings a day' = "2 to 4 servings/day", 'More than 4 servings a day' =  "More than 4 servings/day") 
full_df_PA_v2$cooked_vg %>% table


full_df_PA_v2$fresh_fruit %>% table
full_df_PA_v2$fresh_fruit <- factor(full_df_PA_v2$fresh_fruit)
levels(full_df_PA_v2$fresh_fruit)
levels(full_df_PA_v2$fresh_fruit) <- list( 'Less than 2 servings a day' ="< 2 servings/day", 'Between 2 and 4 servings a day' = "2 to 4 servings/day", 'More than 4 servings a day' =  "More than 4 servings/day") 
full_df_PA_v2$fresh_fruit %>% table


full_df_PA_v2$smoking %>% table 
full_df_PA_v2$smoking<- factor(full_df_PA_v2$smoking)
levels(full_df_PA_v2$smoking)
levels(full_df_PA_v2$smoking) <- list( 'Never' = "Never", 
                                    'Previously' = "Previous", 
                                    'Currently' =  "Current") 


full_df_PA_v2$BMI %>% table 
full_df_PA_v2$BMI <- factor(full_df_PA_v2$BMI)
levels(full_df_PA_v2$BMI)
levels(full_df_PA_v2$BMI) <- list( "Underweight (< 18.5 kg/m2)" = "Underweight (< 18.5 kg/m2)",
                                "Normal (18.5 kg/m2 to < 25 kg/m2)" = "Normal (18.5 kg/m2 to < 25 kg/m2)" ,
                                "Overweight (25 kg/m2 to < 30 kg/m2)" = "Overweight (25 kg/m2 to < 30 kg/m2)",
                                "Obesity Class I, II or III (> 30 kg/m2)" = "Obesity Class I, II or III (> 30 kg/m2)") 

full_df_PA_v2$alcohol %>% table 
full_df_PA_v2$alcohol <- factor(full_df_PA_v2$alcohol)
levels(full_df_PA_v2$alcohol)
levels(full_df_PA_v2$alcohol) <- list( "Never" = "Never", 
                                    "Less than once a week" = "Less than once a week", 
                                    "Once or twice a week" =  "Once or twice a week",
                                    "Three or four times a week" = "Three or four times a week",
                                    "Daily or almost daily" = "Daily or almost daily") 
...
full_df_PA_v2$avg_hh_income %>% table 
full_df_PA_v2$avg_hh_income <- factor(full_df_PA_v2$avg_hh_income)
levels(full_df_PA_v2$avg_hh_income)
levels(full_df_PA_v2$avg_hh_income) <- list( "Less than 18,000" = "Less than 18,000",
                                          "18,000 to 30,999" = "18,000 to 30,999",
                                          "31,000 to 51,999" = "31,000 to 51,999",
                                          "52,000 to 100,000" = "52,000 to 100,000",
                                          "Greater than 100,000" = "Greater than 100,000",
...
full_df_PA_v2$processed_meat %>% table 
full_df_PA_v2$processed_meat <- factor(full_df_PA_v2$processed_meat)
levels(full_df_PA_v2$processed_meat)
levels(full_df_PA_v2$processed_meat) <- list( "Less than 2 times a week" = "Less than 2 times a week", 
                                    "2-4 times a week" = "2-4 times a week", 
                                    "More than 4 times a week" =  "More than 4 times a week") 

full_df_PA_v2$oily_fish %>% table 
full_df_PA_v2$oily_fish <- factor(full_df_PA_v2$oily_fish)
levels(full_df_PA_v2$oily_fish)
levels(full_df_PA_v2$oily_fish) <- list( "Less than 2 times a week" = "Less than 2 times a week", 
                                    "2-4 times a week" = "2-4 times a week", 
                                    "More than 4 times a week" =  "More than 4 times a week") 

full_df_PA_v2$non_oily_fish %>% table 
full_df_PA_v2$non_oily_fish <- factor(full_df_PA_v2$non_oily_fish)
levels(full_df_PA_v2$non_oily_fish)
levels(full_df_PA_v2$non_oily_fish) <- list( "Less than 2 times a week" = "Less than 2 times a week", 
                                    "2-4 times a week" = "2-4 times a week", 
                                    "More than 4 times a week" =  "More than 4 times a week") 

full_df_PA_v2$red_meat %>% table 
full_df_PA_v2$red_meat <- factor(full_df_PA_v2$red_meat)
levels(full_df_PA_v2$red_meat)
levels(full_df_PA_v2$red_meat) <- list( "Less than 2 times a week" = "Less than 2 times a week", 
                                    "2-4 times a week" = "2-4 times a week", 
                                    "More than 4 times a week" =  "More than 4 times a week") 

full_df_PA_v2$education_level %>% table 
full_df_PA_v2$education_level <- factor(full_df_PA_v2$education_level)
levels(full_df_PA_v2$education_level)
levels(full_df_PA_v2$education_level) <- list( "None of the above" = "None of the above", 
                                    "O levels/GCSEs or equivalent, CSEs or equivalent" =  "O levels/GCSEs or equivalent, CSEs or equivalent",
                                    "A levels/AS, NVQ/HND/HNC or equivalent" = "A levels/AS levels or equivalent, NVQ or HND or HNC or equivalent, other professional qualifications",
                                    "College or University degree" = "College or University degree") 
...
dt <- full_df_PA %>%  dplyr::select(
 ...
 apo_a_0,
 med_chol_BP_diab,
 med_chol_BP_diab_or_exohormones
  ) 


# Create variable that indicate if the participant has had CVD or not
dt <- dt %>%
  mutate(CVD = ifelse(MI == 1 | stroke == 1, "Had CVD", "Never had CVD"))
...
# Create table - 2 columns separating people who have had a CVD or not
tb <- dt %>%
  dplyr::select(-contains("truncated")) %>%
dplyr::select(-MVPA_Quant_PA1, -MVPA_Quant_PA2, -MVPA_Quant_PA3, -`2_years_survival_stroke`, -`2_years_survival_MI`) %>%
  tbl_summary(
    by = CVD, 
...
varNameDf <- data.frame(
  varName = c("Myocardial infarction",
              ...
              "Sex", 
              "Follow up time in months",
              "Follow up time in months for self-reported PA"
             # "Two years stroke survival",
             # "Two years MI survival",              
              #"Greenspace 300m from residence",
              #"Water 300m from residence",
...
tb2 <- dt2 %>%
  dplyr::select(-contains("truncated")) %>%
  dplyr::select(-MVPA_Quant_PA1, -MVPA_Quant_PA2, -MVPA_Quant_PA3, -MVPA_Quant_PA4) %>%
  tbl_summary(
    by = CVD,
    statistic = all_continuous() ~ "{median} ({p25}, {p75})", 
    digits = all_continuous() ~ 1,
...
tb3 <- dt3 %>%
  dplyr::select(-contains("truncated")) %>%
  dplyr::select(-MVPA_Quant_PA1, -MVPA_Quant_PA2, -MVPA_Quant_PA3) %>%
  tbl_summary(
    by = CVD,
    statistic = all_continuous() ~ "{median} ({p25}, {p75})", 
    digits = all_continuous() ~ 1,
...
varNameDf <- data.frame(
  varName = c("Myocardial infarction",
              ...
              "Follow up time in months", 
              "Greenspace 300m from residence",
              "Greenspace 1000m from residence",
              "Water 300m from residence",
              "Water 1000m from residence",
              "Natural env. 300m from residence",
              "Natural env. 1000m from residence",
              "Particulate matter air pollution (pm2.5) absordance",
              "Particulate matter air pollution (pm2.5)",
              "Noise pollution",
             # "Home area population density (urban or rural)",
              "Frequency of travelling from home to job",
...
dt8 <- full_df_PA_v2 %>%  dplyr::select(MI,
                                        stroke,
                                        fu_time, 
                                        MVPA_100,
                                        MVPA_150,
...
varNameDf <- data.frame(
  varName = c("Myocardial infarction",
              "Stroke",
              "MVPA min/week - ENMO (100g)",
              "MVPA min/week - ENMO (150g)",
              "MVPA min/week - Machine learning",
              "MVPA min/week - Activity count",
              "Follow up time in months",
              "Race", 
...
#saveRDS(dt8, "/home/yacine/dataframes_AccelerometerProcessing/prepped_dt8.rds")
